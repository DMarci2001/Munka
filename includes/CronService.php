...
class CronService {
    ...
    public function run() {
        ...
        if ($this->interval == "teszt") {
            require("includes/seeme-gateway-class.php");
        ...
        {
            include ( "includes/google-drive-downloader.php" );
        ...
        if ($this->interval == "fejlesztésalatt!!")
        {
            include ( "includes/google-drive-downloader.php" );
            ...
            $request = sql_query("SELECT * FROM GDPR");

            while ( $file = sql_fetch_array( $request ))
            {
                $key = array_search ( $file['id'].".pdf", array_column ( $files, 'name' ));

                if ( $key !== FALSE )
                {
                    if ( $files[$key]['modifiedtime'] != $file['last_modify'] )
                    {
                        //Drive felülírása a helyes fájlal a szerverről.
                    }
                }
            }
        }
        ...
        if ($this->interval == "perc") {
            ...
            if (!in_array(date("G"),array(22,23,0,1,2,3,4,5))) {
                ...
                while ($row=sql_fetch_array($res)) {
                    ...
                    if ($row["rlang"]=="en") $szoveg="Hungáriamed: You have an appointment - ".substr($row["datum"],11,5)." {$row["cim"]}";
        ...
        if ($this->interval == "1ora")
        {
            if( date( "G" ) == 19 )
            {
                $request = sql_query("SELECT * FROM cegek WHERE alksend = ?", array( 1 ));
