...
- **Engedélyezés** — a hívó szerepkörének (`users.auth`) ellenőrzése minden írás előtt.
...
## 2. Szerepkörök és jogosultsági mátrix
...
- **`it_admin`** (IT-admin) — az IT-csapat. Mindent, **plusz** eszköztípusok és `attribute_definitions` kezelése (a metaadat-séma). *(A telephelyeket — `locations` — a klinika weboldala adja; ez a modul csak olvassa.)*

A szerepkörök rangsora: `user` (1) < `storekeeper` (2) < `it_admin` (3). A `role_at_least(role, min)` ezt a rangsort hasonlítja össze. A `users` **külső** tábla; ez a rendszer csak **olvassa** az `auth`-ot, sosem írja — nincs `grant_role` művelete.
...
| Telephelyek (`locations`) — külső (csak olvasás)       | —    | —           | —        |
...
### 3.1 `register_device(actor, payload)` — storekeeper / it_admin

1. Engedélyezés: szerepkör ≥ storekeeper.
2. Egy `devices` sor beszúrása (közös mezők; `status = 'Kivehető'`, `created_by = updated_by = actor`). Az **`asset_tag`** — a QR-kódba kódolt, **egyedi** leltári azonosító (pl. `ESZ-0001`) — **kötelező**: üres értéknél a kezelő hibát ad („Adj meg leltári azonosítót."), ütközésnél elutasít (DB-szinten az `uq_devices_asset_tag` egyediségi megszorítás kényszeríti ki). Ez az az azonosító, amelyet a QR-beolvasásos kivétel/visszavétel élő állapotra old fel.
...
### 3.2 `edit_device(actor, device_id, changes)` — storekeeper / it_admin

Frissíti a közös mezőket (köztük az **`asset_tag`**-et, amelynek egyedinek kell maradnia) és/vagy a típusspecifikus értékeket; beállítja: `updated_by = actor`, `updated_at = now()`. A birtoklást nem érinti.

### 3.3 `retire_device(actor, device_id, reason)` — storekeeper / it_admin
...
### 3.5 Birtoklási és életciklus-műveletek
...
## 4. A birtoklási kezelő (`move_asset`)

Ez a user-oldali birtoklási műveletek (`check_out`, `check_in`, `transfer`) egyetlen kapuja, így a szabályok egy helyen élnek.
...
### 4.3 A folyamatok a gyakorlatban
...
## 5. Űrlapgenerálás és értékellenőrzés
...
## 8. Checkpoint kezelők — `confirm_check_in` / `reject_check_in`
...
### 8.2 `confirm_check_in(actor, event_id)` — storekeeper / it_admin
...
### 8.3 `reject_check_in(actor, event_id, reason)` — storekeeper / it_admin
...
## 9. Felhasználói foglalás (reservation)
...
### 9.1 `reserve_device(actor, device_id, notes=null)` — bármely bejelentkezett felhasználó
...
### 9.2 `cancel_reservation(actor, device_id)` — a foglaló vagy storekeeper / it_admin

1. Engedélyezés: az aktor a `reserved_by`, vagy szerepkör ≥ storekeeper.
2. A sor törlése.
3. `devices.status = 'Kivehető'`.

### 9.3 Elvitel — `check_out` interplay (a `move_asset`-ben)
...
### 9.4 Lejárat — `expire_reservations()` (ütemezett)
...
## 10. Szerviz- és elveszett-kezelők — raktáros+
...
### 10.1 `send_to_repair(actor, device_id, to_location_id=null, to_department_id=null, notes=null)`
...
### 10.2 `return_from_repair(actor, device_id, to_location_id, to_department_id, notes=null)`
...
### 10.3 `mark_lost(actor, device_id, notes=null)`
...
### 10.4 `mark_found(actor, device_id, to_location_id, to_department_id, notes=null)`
