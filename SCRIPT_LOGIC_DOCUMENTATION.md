...
- **Engedélyezés** (mit tehetsz itt): a modul végzi a `users.auth` alapján. Hiányzó/`NULL` érték esetén az alapértelmezés `user`.

```text
role_of(user_id):
    return SELECT auth FROM users WHERE id = :user_id
