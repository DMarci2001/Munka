...
if (isset($_GET["szerk"])) {
	...
	/*
	echo "<tr><td width='100'>Város:</td><td>";
	echo "<select name='townid'>";
	$res=sql_query("select * from towns");
	echo "<option value='0'>Nem kapcsolódik városhoz</option>";
	while ($rowt=sql_fetch_array($res)) {
		echo "<option value='{$rowt["id"]}'".($_POST["townid"]==$rowt["id"]?" selected":"").">{$rowt["townname"]}</option>";
	}
	echo "</select>";
	echo "</td></tr>";
	echo "<tr><td width=100>Rovat:</td><td>";
	echo "<select name='tipus'>";
	echo "<option value='1'".($_POST["tipus"]==1?" selected":"").">Események</option>";
	echo "<option value='2'".($_POST["tipus"]==2?" selected":"").">Hírek</option>";
	echo "</select>";
	echo "</td></tr>";
	*/
	echo "<tr><td width=100>Dátum:</td><td><input class=inputbox style='width:110px;' type=text name=datum value='".substr($_POST["datum"],0,16)."'></td></tr>";

	/*
	$ck=str_replace("||",", ",$_POST["cimkek"]);
	$ck=str_replace("|","",$ck);
	

	echo "<tr><td>Cimkék:</td><td valign=top><input placeholder='cimkék vesszővel elválasztva' class='inputbox' style='width:600px;' type='text' name='cimkek' value='".@$ck."'></td></tr>";
	*/
	...
	echo "<br><input type='submit' name='dokmentes' value='Mentés'> ";
	...
	echo  "<a name=kepek></a>Képfeltöltés: &nbsp;<input size=50 type=file name=kepfile> <input type='submit' name='dokmentes' value='Feltöltés'><br><br>";
	$x=0;
	while ($rowp=sql_fetch_array($resp)) {
		...
		if (substr(strtolower($rowp["filename"]),-4)==".jpg") {
...
if (!isset($_GET["szerk"])) {
	...
	WHERE c.tipus=10 ORDER BY datum DESC");

	echo "<table cellpadding=0 cellspacing=0 border=0>";
	while ($row=sql_fetch_array($res)) {
		if (trim($row["cim"])=="") $row["cim"]="Új bejegyzés";
		$tc="tcella";
		if (!isset($first)) {
		...
		echo "<td nowrap valign=top><div class={$tc}>{$row["townname"]}&nbsp;&nbsp;</div></td>";
		...
		echo "<td nowrap valign=top><div class={$tc}>[<a onclick='return confirm(\"Biztosan törlöd ezt a filet?\");' href='{$_SERVER["PHP_SELF"]}?page={$_GET["page"]}&delete={$row["id"]}'>delete</a>]</div></td>";
		echo "</tr>";
		echo "<tr><td colspan=7 style='border-top:1px solid #ccc;height:1px;'></td></tr>";
	}
	echo "</table>";
