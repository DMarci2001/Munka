...
function setHelyszin2(h) {
	window.location.href='index.php?sethelyszin2='+h;
}
...
function showFizSzolg(id) {
	$("#fizszolglist"+id).load("index.php?showfizszolglist&fid="+id);
}
...
function addFizSzolg(fid,aid) {
	$.ajax({
		type: "POST",
		url: "index.php",
		data: "addfizszolg=1&fid="+fid+"&aid="+aid,
		success: function(data)	{
...
function removeFizSzolg(fid,id) {
	...
	$.ajax({
		type: "POST",
		url: "index.php",
		data: "removefizszolg=1&fid="+fid+"&id="+id,
		success: function(data)	{
			$("#fizszolglist"+fid).html(data);
		}
	});
...
function setListDay(day) {
	...
	$("#elojegyzestable").load("index.php?page=elojegyzestabla&showelojegyzestable&day="+encodeURIComponent(day),null,
		function(responseText){
...
function addIdopont(idopont,szt) {
	if (foglalasSelected!=0) {
		...
		if (confirm(msg)) {
			let params = "?page=elojegyzestabla&cpy="+cpy+"&szt="+encodeURIComponent(szt)+"&moveidopont="+encodeURIComponent(idopont)+"&fid="+encodeURIComponent(foglalasSelected)+"&rinterval="+selectedInterval;
			$("#elojegyzestable").load("index.php"+params,null,
				function(responseText){
					if (cpy==0) showIdopontEditor('elojegyzestabla',foglalasSelectedPass,foglalasSelected);
    ...
    $.ajax({
        url:'index.php',
        type:'GET',
        data:{page:'elojegyzestabla', szt:szt, addidopont:idopont, rinterval: selectedInterval},
        success:function(data){
...
function removeIdopont(id) {
	...
	$("#elojegyzestable").load("index.php?page=elojegyzestabla&removeidopont="+encodeURIComponent(id),null,
		function(responseText){
...
function removeIdopontNaptar(id,idopont) {
	...
	$("#foglalasnaptaridopont").load("index.php?page=bnaptar&removeidopont="+encodeURIComponent(id)+"&idopont="+encodeURIComponent(idopont),null,
		function(responseText){
			if (foglalasDisplayed==id) {
				cancelFoglalasMove();
				$("#idoponteditor").slideUp();
...
function foglalasMentes(page) {
	...
	$.ajax({
		...
		success: function(response)	{
			...
			$("#elojegyzestable").load("index.php?page=elojegyzestabla&showelojegyzestable");
		}
	});
