...
$w=$bw="";
if ($user["jogosultsag"]<2 || $user['orvosid']!="") {
	if (isset($user["cegjog"])) {
		$w="and cegid in (".getCegList($user["cegjog"]).")";
		$bw="and id in (".getCegList($user["cegjog"]).")";
	}
}


if (!isset($_SESSION["cegfilter"])) $_SESSION["cegfilter"]=0;
if ($_SESSION["cegfilter"]>0) $w.="and u.cegid='".addslashes($_SESSION["cegfilter"])."'";
if ($_SESSION["cegfilter"]==-1) $w.="";

if (!isset($_SESSION["kereskulcs"])) $_SESSION["kereskulcs"]="";
if (isset($_POST["kereskulcs"])) $_SESSION["kereskulcs"]=$_POST["kereskulcs"];
...
if (isset($_GET["szerk"])) {
	...
	$row=sql_fetch_array(sql_query("select u.*,c.megnev as cegnev from felhasznalok u left join cegek c on c.id=u.cegid where u.id='".addslashes($_GET["szerk"])."' {$w}"));
	$_POST=$row;
	...
	if ($user["jogosultsag"]>=2 || $user['jog_leletlatas']==1) {
		...
		<div class = "tdsepdiv" style = "margin-top:20px;">
		Leletek
		</div>
		<div id = "lelet-lista" style = "color:#aaa"><?php echo leletLista( $patient_id, $_GET['page'], $_GET['szerk'] ) ?></div>
		<div id = "leletform" style = "color:#000" class = "option-box"></div>
		<div id = "zaroform" style = "color:#000" class = "option-box"></div>
		<div id = "leletbutton" style = "margin-top:10px;">
		<?php if($_SESSION['adminuser']['jog_leletlatas'] == 1) { ?>
		<input value = "+ Lelet készítése" id = "add-lelet" name = "uj_lelet" type = "button"/>
		<?php } ?>
		</div>
		<?php
	}
...
if (isset($_GET["fszerk"])) {
	...
	if ($id==0) {
		...
		echo "<h2>Felhasználó szerkesztése</h2>";
	}
	...
	if ($id!=0) {
		...
		if (isset($_POST["cegid"])) $selected=$_POST["cegid"];
	...
	if ($_SESSION["cegfilter"]==15) echo " [<a onclick='alert(\"TAJ szám nem található a törzs adatok között!\");' href='#'>Adatok automatikus kitöltése</a>]";
	echo "</td></tr>";
...
if (!isset($_GET["szerk"]) && !isset($_GET["fszerk"])) {

	//if ($user["jogosultsag"]>=2) {
		
		if($user['jog_vizsg_stat'] == 1){
		...
		echo "<select name='cegselect' onchange='setCegFilter(this.value,\"felhasznalok\");'>";
		echo "<option value='0'>Szűrés cégre</option>";

		$res=sql_query("SELECT * FROM cegek where true {$bw} order by megnev");
	
		if (sql_num_rows($res)>1) echo "<option value='-1'".($_SESSION["cegfilter"]==-1?" selected":"").">Összes cég</option>";
	...
	if (isset($_SESSION["kereskulcs"]) && $_SESSION["kereskulcs"]!="") {
		...
		$w.="and ( instr(u.nev,'{$kulcs}') or instr(u.taj,'{$kulcs}') or instr(u.torzsszam,'{$kulcs}') or instr(u.szuldatum,'{$kulcs}'))";
	...
	if($_GET['scroll'] > count($page) || $_GET['scroll'] < 0) header("Location:index.php?page=felhasznalok&scroll=1");
		...
		<?php
			...
			foreach($page as $key => $value)
			{
				...
				if( ($_GET['scroll'] - 10 ) > $key )
				{
					...
					$pageout.= "<a class = 'ujbutton' href = 'index.php?page=felhasznalok&scroll=1' {$aStyle} >1.</a>&nbsp;";
					$pageout.= "...&nbsp;";
					$preHide++;
					continue;
				}
				$pageout.= "<a class = 'ujbutton' href = 'index.php?page=felhasznalok&scroll={$page[$key]['number']}' {$aStyle} >{$page[$key]['number']}.</a>&nbsp;";
				//Ha lapszámhoz képest 8 értékkel nagyobb lapokat rejtse le, de az utolsót mutassa.
				if( $key == ( $_GET['scroll'] + 8 ))
				{
					$pageout.= "...&nbsp;";
					$pageout.= "<a class = 'ujbutton' href = 'index.php?page=felhasznalok&scroll=".count($page)."' {$aStyle} >".count($page).".</a>&nbsp;";
					break;
				}
			}
			echo $pageout;
