...
function getIntervals($beosztasok) {
    $intervals = [];
...
function showElojegyzesTable($setday) {
	...
	for ($i=-60;$i<150;$i++) {
		...
		$htmlout.= "<option value='{$day}'".($day==$setday?" selected":"").">".magyarDatum($day)."</option>";
	}
	$htmlout.= "</select>";
	$htmlout.="</div>";
	...
	while ($szuresTipus=sql_fetch_array($szuresTipusok)) {
		...
		$intervals = getIntervals($beosztasok);
        ...
        foreach ($intervals as $binterval) {
            ...
            foreach ($beosztasok as $beosztas) {
                ...
                $orvosok .= "<div style='width:250px;'><a target='_blank' href='{$_SERVER['PHP_SELF']}?page=orvosok&szerk={$beosztas["orvosid"]}'>{$beosztas["orvosnev"]}</a> - " . (substr_count($beosztas["cegek"], ",") > 0 ? "<a onclick='$(\"#beocegek{$beosztas["id"]}\").slideToggle();return false;' href='#'>" . (substr_count($beosztas["cegek"], ",") + 1) . " cég</a>" : substr_jns($beosztas["cegek"], 0, 20)) . "{$szabiURL}</div>";
            ...
            $maxrendeles = 0;
            //load napi beosztás
            if ($beos = getBeosztasok($nap, $helyszin, $szuresTipus["id"])) {
                foreach ($beos as &$beo) {
            ...
            if ($minTol != "24:00") {
                ...
                for ($o = 0; $o < 3600; $o += $binterval) {
                    ...
                    while ($rowf = sql_fetch_array($resf)) {
                        ...
                        if ($orvNev == "" && $cegNev == "") $htmlout .= "???";
                        $htmlout .= "&nbsp;&nbsp;";
                        //$fizSzolg=showFizSzolg($rowf["id"]);

                        $htmlout .= "<div id='fiz_szolglist{$rowf["id"]}'>" . showFizSzolg($rowf["id"], 1) . "</div>";
