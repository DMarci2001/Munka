...
class TeladocApi {
    //győrből átmásolva, még nincs használatban

    const LOCATION_ID = 176;
    const PROVIDER_NAME = "teladoc";
    const LOG_ID = 30;
    ...
    private function locations():array {
        ...
        foreach ($locations as $location) {
            $phones = ["+36 20 373 3343"];
            $locationArray[] = [
                "id" => $location["id"],
                "name" => "Hungariamed Győr",
                "zip" => "9021",
                "city" => "Győr",
                "address" => "Bajcsy-Zsilinszky utca 30-32.",
                "phone" => $phones,
                "email" => "gyor@hungariamed.hu",
                "website" => "https://gyor.hungariamed.hu",
                "modifiedAt" => date("c")
            ];
        }
    ...
    private function token():array {
        ...
        if (!$tokenData = sql_fetch_array(sql_query("select * from tokens where username=? and password=md5(?)", [$this->postParams["username"], $this->postParams["password"]]))) {
            $this->apiError(400, "invalid grant", "The user name or password is incorrect.");
        }
    ...
    private function reservationsGet():array {
        ...
        $return = [];
        $reservations = sql_query("select * from foglalasok where datum>=? and datum<=? and source=? {$doctorFilterReservation} {$specializationFilter} {$locationFilter}", [$startDate, $endDate, self::PROVIDER_NAME])->fetchAll(PDO::FETCH_ASSOC);
    ...
    private function _reservationArray($reservationData):array {
        return [
            ...
            "authorizationCode" => $reservationData["teladoccode"],
    ...
    private function reservationsPost():array {
        ...
        $length             = $body["length"];
        $cegId              = Booking_Constants::COMPANY_HMM_GYOR;
    ...
    private function calculateSlots($startDate, $endDate, $locationId, $specializationId, $doctorId):array {
        ...
        $beoDatas = sql_query("SELECT b.*, o.id as orvosId, o.nev FROM orvos_beosztas b
            ...
            WHERE ((nap = 10 AND beonap>=? AND beonap<=?) OR nap<10) AND nap<>0 {$specializationFilter} {$doctorFilter} AND o.unionid<>0", [date("Y-m-d", strtotime($startDate)), date("Y-m-d", strtotime($endDate))])->fetchAll(PDO::FETCH_ASSOC);
