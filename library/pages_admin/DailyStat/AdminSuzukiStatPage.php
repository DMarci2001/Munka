...
class AdminSuzukiStatPage extends AdminCorePage
{
     ...
     */
    private function search_for_bottleneck($packages, $date, $beo)
    {
        ...
        foreach ($packages as $package) {
            ...
            $bookings = sql_query(
                "SELECT id,szurestipusid,orvosassigned,datum FROM foglalasok WHERE szurestipusid IN(".implode(",", array_column($types, "szurestipusid")).") AND datum BETWEEN ? AND ?",
            ...
            foreach ($types as $key => $value) {
                $schedules[$value["szurestipusid"]] = sql_query(
                    "SELECT tol as startTime,ig as endTime,binterval,orvosid,beonap,nap 
            ...
            foreach($schedules as $examination=>$schedule_properties){
                ...
                foreach($doctors as $doctor){
                    ...
                    echo "vizsgáló orvos: {$doctor}<br>";
                    if($doctor==1019){
                        echo "lefoglalt időpontok: <br>";
                        //$this->debug_array($bookedTimes);
                    }
                }
