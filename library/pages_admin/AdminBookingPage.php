...
class AdminBookingPage extends AdminCorePage
{
    ...
    public function showPage()
    {
       ...
       // doctorid=2 carespotid=1
        ...
        /*$csomagok = [];
        $csaktetel = 0;
        $q=sql_query("SELECT nev,szuldatum,taj,laborpacks FROM labrequests l
                      WHERE bekuldokod='000000481' AND INSTR(created,'2025') and status='done';")->fetchAll(PDO::FETCH_ASSOC);
        echo "Összes laborkérő ami valid: ". count($q)."<br>";
        $x = 0;
        foreach($q as $l){
            $packs = json_decode($l["laborpacks"]);
            if(empty($packs)){
                $csaktetel++;
            }
            foreach($packs as $pack){
                $key = array_search($pack,array_column($csomagok,"csomagid"));
                if($key!==false){
                    $csomagok[$key]["db"] = ($csomagok[$key]["db"]+1);
                }else{
                    $csomagok[] = [
                        "csomagid"=>$pack,
                        "db"=>1,
        ...
        echo "<table>";
        echo "Csak tétel vásárlások: {$csaktetel}db<br>";
        echo "<tr><td>Csomag</td><td>darab szám</td></tr>";
        foreach($csomagok as $csomag){
            $a = sql_query("SELECT * FROM synlab_labor_csomagok WHERE id=?",[$csomag["csomagid"]])->fetch(PDO::FETCH_ASSOC);
            echo "<tr>";
            echo "<td>{$a["name"]}</td>";
            echo "<td>{$csomag["db"]}</td>";
            echo "</tr>";
        }
        echo "</table>";*/
        ...
        $teljescimRaw = explode(" ", $r["teljes_cim"]);
            ...
            unset($teljescimRaw[0], $teljescimRaw[1]);
            foreach ($teljescimRaw as $key => $value) {
                if (empty($value)) {
                    unset($teljescimRaw[$key]);
                }
            }
            $teljescimRaw = array_values($teljescimRaw);
            $cim = implode(" ", $teljescimRaw);


            $teljescim = array("irsz" => $irsz, "varos" => $varos, "cim" => $cim);
            sql_query(
                "UPDATE dokirex_insert_paciensek SET Iranyitoszam=?, Telepules=?, Cim=? WHERE id=?",
                array($teljescim["irsz"], $teljescim["varos"], $teljescim["cim"], $r["id"])
            );
        ...
        /*if($exist = sql_query("SELECT * FROM felhasznalok WHERE taj=? AND cegid=?",array($r["taj"],$cegId))->fetch(PDO::FETCH_ASSOC)){
            ...
            sql_query(
                "INSERT INTO felhasznalok SET nev=?,neme=?,cegid=?,szid=?,telefon=?,anyjaneve=?,szulhely=?,szuldatum=?,taj=?,email=?,
                                                    irsz=?,varos=?,utca=?,regtime=?,validated=?,statusz=?",
                array(
                    $r["nev"], $r["neme"], $cegId, $r["szid"], $r["telefon"], $r["anyjaneve"], $r["szulhely"], $r["szuldatum"], $r["taj"], $r["email"],
        ...
        /*$params = array(
                ...
                "Azonosito" => $r["taj"],
                "AzonositoTipusID" => 2,
                "SzuletesiDatum" => $r["szuldatum"],
                "SzuletesiHely" => $r["szulhely"],
                "AnyjaNeve" => $r["anyjaneve"],
                "Nem" => $r["neme"],
                "SzuletesiNev" => $r["nev"],
                "Allampolgarsag" => "109",
                "Telefon" => $r["telefon"],
                "Mobiltelefon" => $r["telefon"],
                "Iranyitoszam" => $r["Iranyitoszam"],
                "Telepules" => $r["Telepules"],
                "Cim" => $r["Cim"],
                "Email" => $r["email"],
                "SzigSzam" => null,
                "KozgyogyTol" => null,
        ...
        /*$params = array(
            ...
            $params = array(
                ...
                "PaciensID"=>$fogl["dokirex_userid"],
738","095619628","040007605","086496197","026720629","023346376","032358449","029387562","032294408","025496367","027364693","027729120","031359900","040722825","023420045","031146191","025736841","044569934","041196832","039833925","107549262","024207717","026021933","043446384","037274098","036510698","028652171","034041600","043416457","112834360","030149625","094958920","044273343","024359438","031810113","041856626","115972807","033209399","031887742","030728459","095029629","025075397","110190839","030920392","027732612","040465724","026662006","027479100","034451924","081605518","091180825","042660004","023876480","031241416","031126889","023290231","108627327","044526986","032889749","038798382","026065744","032138597","091461975","034389403","035444145","111157688","035817901","094609244","029621989","091243508","031929116","085730896","112189378","039111366","086844213","036379190","037536039","041360385","033329127","028381347","023476516","037260420","112089261","035451350","095018492","030465985","026134422","038332377","093469403","028739164","027171574","027878541","036419261","076768134","025886094","030241671","029589711","117037999","027525159","018892871","043935293","116310181","128332074","039389996","095071518","023922934","043426838","042799900","027434086","028942186","116295028","106017003","112660415","121903145","045258211","038083581","069889617","110566971","032057063","043900873","031715087","038744774","034559099","108124266","031947730","110602707","045106576","027403224","033482790","029586583","044913148","028796611","044910769","045061125","040923699","120404649","118780382","115550861","120996779","035170987","109147228","118208327","025672178","085184158","033695664","039283557","095105055","105403911","030958373","033729998","108411809","083076936","030401233","022674661","025957570","033335005","030133486","122803495","044479947","045124190","037453477","044048259","114386133","116631549","120601219","031584348","042511384","042123053","114133322","033578040","024548054","118277967","033695066","038089497","121327680","027441404","077233547","042911760","038047714","086161123","123896984","036540343","111101935","040284293","035853644","114910156","117215577","115607082","117585043","032653500","115032046","119334627","036631100","035968096","041782905","027926381","111612325","038507432","032934894","108110612","079865098","026143143","044893406","036120521","033878515","110266712","033721480","033271057","119057812","031989152","037125756","033328522","031332503","037373483","128781427","124259269","122941036","120958166","027906990","121624154","027630099","031743886","121535353","036142370","031801492","028305138","040161969","037744362","106273636","034926217","123183981","036117101","095178387","110846770","036906042","108593495","037716745","032579460","029604221","044656676","031657732","114153421","045271861","029110386","044516529","039291051","027350197","040202903","109239958","028639497","032281875","116638067","030946493","043758210","107702966","045293421","043054677","032681770","125784481","028480282","109280194","044449199","044555380","094693782","037546104","027017203","111304208"];
    ...
    private function elojegyzesTableRow($reservationData, $ora, $binterval, $noAdd = false): string
    {
        ...
        $trStyle = "";
        if (in_array($reservationData["taj"], $this->vipList)) {
            $trStyle = "background-color:yellow;font-weight:bold;margin-top:5px";
        }
        if (in_array($reservationData["taj"], $this->vizmuvekList)) {
            $trStyle = "background-color:lightgreen;font-weight:bold;margin-top:5px";
        }

        $htmlout .= "<tr style='{$trStyle}'>";
        ...
        if ($jogosult) {
            ...
            if (in_array($reservationData["taj"],$this->vipList)) {
                $vipSign = "<i class='fa-solid fa-crown'></i>";
            }
            if (in_array($reservationData["taj"],$this->vizmuvekList)) {
                $vipSign = "&nbsp;<i class='fa-solid fa-water'></i>&nbsp;";
            }
            ...
            $htmlout .= "{$vipSign}<a id='det{$reservationData["id"]}' onclick='{$detailURL}' href='#' style='" . ($reservationData["nev"] == "Foglalt" ? "color:#aaa;" : "") . "'>{$reservationData["nev"]}</a>{$vipSign}{$kidSign}{$tudoszuroSign}{$hallasSign}{$laborSign}{$docSign}&nbsp;&nbsp;";
