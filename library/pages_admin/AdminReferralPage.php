		...
		if(isset($_POST['saveTest']) || isset($_POST["sendSelectedReferrals"])){
			...
			if (isset($_POST['selected-data']) && is_array($_POST['selected-data']) && count($_POST['selected-data'])>0) {
				...
				foreach ($_POST['selected-data'] as $selector) {
					...
					$mbody.= "<p style='font-family:calibri;font-size:14px'>Az alkalmassági eredményt kérjük <strong>Lente Gyöngyi részére elküldeni</strong> a <a href=\"mailto:lente.gyongyi@opusenergetika.hu\" style=\"color:#a90000\">lente.gyongyi@opusenergetika.hu</a> e-mail címre.</p>";
				...
				if (isset($_POST['saveTest'])) {
					...
					$mail->AddAddress("lente.gyongyi@opusenergetika.hu");
				}
